    ...
    Scaffold(
        ...
        if (showClearDialog) {
            AlertDialog(
                ...
                text = {
                    Text(
                        "Все мутации (включая «застрявшие» в конфликте) будут удалены, " +
                            "флаги conflictPending у локальных приёмок снимутся. Локальные " +
                            "записи не удаляются — следующий sync подтянет актуальное " +
                            "состояние с сервера. Используйте при «зомби»-очереди."
                    )
                },
                confirmButton = {
                    TextButton(onClick = {
