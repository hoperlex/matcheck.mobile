...
fun EditableMaterialsInlineList(
    ...
    //
    // Элемент nullable, потому что список 2 Этапа рисуется блоками по
    // документам машины: в блок попадают позиции с произвольными индексами
    // исходного списка, и «оригинала нет» для отдельной строки внутри блока
    // выразить обрезкой по длине нельзя. Прежние вызовы передают
    // List<MaterialDraft> и продолжают компилироваться — List в Kotlin
    // ковариантен.
