 ...
 *
 * Контракт с вызывающим: 401, дошедший наверх, означает «не сейчас», а не
 * «никогда» — ни одна ветка ниже не говорит, что запрос бессмысленен по существу.
 * Очередь мутаций обязана такой ответ повторять, а не выбрасывать. Отправка под
 * чужим токеном при этом невозможна: сменить аккаунт с непустой очередью не даёт
 * барьер входа (PendingWorkProbe считает и pending, и замороженные мутации).
 *
    ...
    override fun authenticate(route: Route?, response: Response): Request? {
        ...
        return runBlocking {
            when (val outcome = coordinator.obtainFresh(storedAccessAtFailure, path)) {
                ...
                // «Повторит» здесь не фигура речи: очередь мутаций трактует 401
                // как transient и оставляет операцию в очереди (httpDisposition).
                // Раньше она его дропала, и завершённый инспектором 2 Этап
                // терялся навсегда — см. MutationProcessor.classifyFailure.
